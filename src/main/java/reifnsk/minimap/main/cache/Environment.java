...
public class Environment {
    ...
    private static void calcEnvironment(int x, int z, Environment environment) {
        if(!environment.valid) {
            ...
            int grassColor = CompatHelper.GET.getGrassColor(world, x, z);
            int foliageColor = CompatHelper.GET.getFoliageColor(world, x, z);
            environment.set(x, z, temperature, humidity, biome, grassColor, foliageColor);
            environment.valid = true;
        }
    }

    public static void setWorld(World worldK) {
        world = worldK;
        queue.clear();
