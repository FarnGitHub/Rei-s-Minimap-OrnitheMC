...
import reifnsk.minimap.compat.CompatHelper;
...
@SuppressWarnings({"unused", "FieldMayBeFinal", "FieldCanBeLocal", "BusyWait", "ResultOfMethodCallIgnored", "CallToPrintStackTrace", "SameParameterValue"})
public class ReiMinimap implements Runnable {
	...
	public void onTickInGame(Minecraft mc) {
		...
		try {
			...
			if(bgbList == null) {
				bgbList = CompatHelper.GET.getBiomes().toArray(new Biome[0]);
			}
		...
		if(id != 0 && (!this.hideSnow || id != 78)) {
			...
			if(this.lightType == 0) {
				...
				switch (this.lightmap) {
					...
					case 1:
					case 2: {
						i11 = y < this.getWorldHeight() ? chunk.getLight(LightType.SKY, x, y + 1, z) : 15;
					}
				};
				...
				switch (this.lightmap) {
					...
					case 0:
						i11 = y < this.getWorldHeight() ? chunk.getLight(x, y + 1, z, this.skylightSubtracted) : 15 - this.skylightSubtracted;
						break;
					case 1:
						i11 = y < this.getWorldHeight() ? chunk.getLight(x, y + 1, z, 0) : 15;
						break;
					case 2: {
						i11 = y < this.getWorldHeight() ? chunk.getLight(x, y + 1, z, 11) : 4;
					}
				};
	...
	public void saveOptions() {
		...
		try {
			PrintWriter printWriter2 = new PrintWriter(new FileWriter(file1));
	...
	public void loadWaypoints() {
		...
		for(int i4 = 0; i4 < i5; ++i4) {
			...
			if(matcher7.matches()) {
                ...
                try (Scanner scanner10 = new Scanner(new File(directory, string3))) {
	...
	public int getWorldHeight() {
		return CompatHelper.GET.getWorldHeight(theWorld);
	}
