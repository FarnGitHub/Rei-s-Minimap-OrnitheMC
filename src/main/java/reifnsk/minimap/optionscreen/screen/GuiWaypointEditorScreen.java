...
import reifnsk.minimap.render.Waypoint;
import reifnsk.minimap.optionscreen.element.GuiScrollbar;
import reifnsk.minimap.optionscreen.element.GuiSimpleButton;
import reifnsk.minimap.optionscreen.element.GuiTextField;
...
public class GuiWaypointEditorScreen extends Screen implements GuiScreenInterface {
	...
	private Screen nonInterfaceParentA;
	...
	public GuiWaypointEditorScreen(Minecraft mc, Waypoint waypoint) {
		...
		if(waypoint == null) {
			name = "";
			ClientPlayerEntity i = mc.player;
	...
	public GuiWaypointEditorScreen(GuiWaypointScreen parrent, Waypoint waypoint, Screen screen) {
		this(parrent, waypoint);
		this.nonInterfaceParentA = screen;
	}
	...
	public void render(int mx, int my, float f) {
		...
		this.drawCenteredTextWithShadow(this.textRenderer, title, this.width / 2, this.height / 2 - 68, -1);
		String temp = Integer.toString(x).equals(this.xCoordTextField.text) ? "xCoord: (Current)" : "xCoord:";
		this.drawTextWithShadow(this.textRenderer, temp, (this.width - 150) / 2 + 1, this.height / 2 - 19, -1);
		temp = Integer.toString(y).equals(this.yCoordTextField.text) ? "yCoord: (Current)" : "yCoord:";
		this.drawTextWithShadow(this.textRenderer, temp, (this.width - 150) / 2 + 1, this.height / 2 - 9, -1);
		temp = Integer.toString(z).equals(this.zCoordTextField.text) ? "zCoord: (Current)" : "zCoord:";
		this.drawTextWithShadow(this.textRenderer, temp, (this.width - 150) / 2 + 1, this.height / 2 + 1, -1);
		...
		this.drawCenteredTextWithShadow(this.textRenderer, String.format("R:%03d", new Object[]{r}), this.width / 2 - 15, this.height / 2 + 21, -2139062144);
		this.drawCenteredTextWithShadow(this.textRenderer, String.format("G:%03d", new Object[]{g}), this.width / 2 - 15, this.height / 2 + 31, -2139062144);
		this.drawCenteredTextWithShadow(this.textRenderer, String.format("B:%03d", new Object[]{b}), this.width / 2 - 15, this.height / 2 + 41, -2139062144);
