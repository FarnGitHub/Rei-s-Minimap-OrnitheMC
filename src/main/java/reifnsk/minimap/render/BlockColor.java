...
public class BlockColor {
	private static final int BLOCK_NUM = BlockRegistry.INSTANCE.size();
			...
			for(int blockID = 0; blockID < BLOCK_NUM; ++blockID) {
				...
				if(block != null) {
					...
					for(int meta = 0; meta < 16; ++meta) {
						...
						try {
							s = TranslationStorage.getInstance().get((new ItemStack(block, 1, meta)).getTranslationKey());
						} catch (Exception exception16) {
						}

						if(s == null || s.isEmpty()) {
							s = TranslationStorage.getInstance().get(block.getTranslationKey());
						}
