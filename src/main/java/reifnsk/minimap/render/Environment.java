...
public class Environment {
	...
	private static OctaveSimplexNoiseSampler temperatureGen;
	private static OctaveSimplexNoiseSampler humidityGen;
	private static OctaveSimplexNoiseSampler noiseGen;
	...
	private void set(int x, int z, double temperature, double humidity) {
		...
		this.foliageColor = FoliageColors.getColor(temperature, humidity);
	}
	...
	private static void calcEnvironment(int x, int z, Environment environment) {
		...
		double humidity = humidityGen.sample((double[])null, (double)x, (double)z, 1, 1, (double)0.05F, (double)0.05F, 0.3333333333333333D)[0];
	...
	public static void setWorldSeed(long seed) {
		if(randomSeed != seed) {
			randomSeed = seed;
			temperatureGen = new OctaveSimplexNoiseSampler(new Random(seed * 9871L), 4);
			humidityGen = new OctaveSimplexNoiseSampler(new Random(seed * 39811L), 4);
			noiseGen = new OctaveSimplexNoiseSampler(new Random(seed * 543321L), 2);
